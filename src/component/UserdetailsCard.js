...
const UserdetailsCard = ({ item }) => {
  ...
  // Group tickets by status and sort each group by priority
  ...
  const sortedGroupedTickets = Object.keys(groupedTickets).reduce((acc, status) => {
    acc[status] = groupedTickets[status].sort((a, b) => b.priority - a.priority);
    return acc;
  }, {});
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <div className='userdet' style={{ display: "flex", justifyContent: "space-evenly", marginLeft: "12px", marginTop: "10px", marginBottom: "-4px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginRight: "30px" }}>
            <img style={{ marginRight: "15px", marginBottom: "-8px", marginLeft: "-10px" }} className='cardimg' src ={`https://api.dicebear.com/5.x/initials/svg?seed=${item.name}`} alt='' />
